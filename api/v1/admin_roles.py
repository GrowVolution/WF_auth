...
async def _get_role(role_id: int, e) -> Role:
    result = await e.exec(select(Role).where(Role.id == role_id))
    role = result.first()
    if not role:
...
async def list_request(_ = resolver("roles:read")):
    ...
    result = await e.exec(select(Role).order_by(Role.name))
    return [_serialize(role) for role in result.all()]
...
async def add_member_request(
    ...
    result = await e.exec(select(User).where(User.id == body.user_id))
    user = result.first()
    if not user:
...
async def remove_member_request(
    ...
    result = await e.exec(select(User).where(User.id == user_id))
    user = result.first()
    if not user:
        raise HTTPException(status_code=404, detail="UNKNOWN_USER")

    if admin and user.id == admin.id and role.is_admin and s.user_service.is_admin(user):
        remaining_admin = any(
